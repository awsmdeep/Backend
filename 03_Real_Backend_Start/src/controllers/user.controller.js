...
const loginUser=asyncHandler(async(req,res)=>{
    ...
    const isPasswordValid=await userCreated.isPasswordCorrect(password)
    if(!isPasswordValid){
        throw new ApiError(404,"password incorrect")
    }

   const {accessToken,refreshToken}= await generateAccessAndRefreshTokens(userCreated._id)
    const loggedInUser=await User.findById(userCreated._id).select("-password -refreshToken")
...
const getCurrentUser=asyncHandler(async(req,res)=>{
    return res.status(200,req.user,"Current user data fetched succesfully")
})
...
const updateUserAvatar=asyncHandler(async(req,res)=>{
    ...
    if(!avatar.url){
        throw new ApiError(400,"error while uolading the file in cloudinary")
    }
    const user=await User.findByIdAndUpdate(
        req.user?._id,
        {
